...
shared_examples_for Mithril::Controllers::Mixins::ActionsBase do
  ...
  describe :define_singleton_action do
    specify { expect(instance).to respond_to(:define_singleton_action).
      with(1..2).arguments.and.a_block }
  end # describe
  ...
  describe "with a singleton action defined" do
    let :command do FactoryGirl.generate :action_key; end
    
    before :each do
      instance.define_singleton_action command do |session, arguments| arguments.join(" "); end
    end # before each
    
    specify { expect(described_class.actions).not_to have_key command }
    specify { expect(described_class.actions true).not_to have_key command }
